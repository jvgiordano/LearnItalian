...
class ImprovedAdaptiveLearningEngine:
    ...
    def calculate_topic_confidence(self, questions_answered):
        """
        Square root confidence curve: Smooth diminishing returns.
        Early questions matter most, with progressively smaller gains.
        Reaches 100% confidence at 14 questions.
        """
        if questions_answered >= 14:
            return 1.0
        
        # Square root curve: sqrt(x) / sqrt(14)
        # This gives: 1q→27%, 2q→38%, 4q→53%, 6q→65%, 10q→85%, 14q→100%
        confidence = (questions_answered ** 0.5) / (14 ** 0.5)
    ...
    def get_coverage_percentage(self, level):
        """Get question-based coverage percentage for a specific CEFR level."""
    ...
    def _calculate_mastery_for_level(self, level, cursor):
        """
        Calculate coverage (question-based) and mastery (topic-based) for a specific level.
        - Coverage: % of questions answered at least once (traditional measure)
        - Mastery: Average topic proficiency with confidence weighting + sigmoid coverage multiplier
        """
        # COVERAGE: Question-based
        cursor.execute("SELECT COUNT(*) AS total FROM questions WHERE cefr_level = ?", (level,))
        ...
        # Count questions answered at least once
        cursor.execute("SELECT id FROM questions WHERE cefr_level = ?", (level,))
        ...
        question_scores_with_attempts = [s for s in mastery_scores.values() if s is not None]
        attempted_count = len(question_scores_with_attempts)
        ...
        for topic in all_topics:
            ...
            if not topic_question_ids:
                topic_mastery_scores.append(0.0)
                continue
            ...
            # Filter out None (unanswered) and apply simple caps
            capped_scores = []
            for score in raw_mastery_scores.values():
                if score is not None:
                    # CHANGED: Cap raw scores at 0.0 (floor) and 1.2 (ceiling)
                    # Negative scores become 0. High scores stay 1.2.
                    capped_scores.append(max(0.0, min(1.2, score)))
            
            if not capped_scores:
                # Topic hasn't been attempted
                topic_mastery_scores.append(0.0)
            else:
                # Calculate topic mastery with confidence multiplier
                avg_question_mastery = sum(capped_scores) / len(capped_scores)
                confidence = self.calculate_topic_confidence(len(capped_scores))
                topic_mastery = avg_question_mastery * confidence
                topic_mastery_scores.append(topic_mastery)
        
        # Average all topic scores (including unseen topics as 0)
        ...
        level_mastery = level_mastery * coverage_multiplier
        
        # Cap final result at 100%
        level_mastery = max(0.0, min(1.0, level_mastery))
        
        return {
            "coverage": f"{coverage*100:.0f}%",
            "mastery": f"{level_mastery*100:.0f}%",
            "coverage_value": coverage,
    ...
    def get_total_coverage_and_mastery(self, cursor):
        ...
        - Global coverage: % of all questions answered (question-based)
        - Global mastery: Average of all level masteries (topic-based)
        """
        # COVERAGE: Question-based across all questions
        ...
        # Use batch query to get all mastery scores
        mastery_scores = self.calculate_all_question_masteries_batch(all_question_ids, cursor)
        
        # Count questions with at least one attempt
        question_scores_with_attempts = [s for s in mastery_scores.values() if s is not None]
        attempted_count = len(question_scores_with_attempts)
        
        global_coverage = attempted_count / total_questions if total_questions > 0 else 0
        
        # MASTERY: Average of all 5 level masteries (topic-based)
        levels = ['A1', 'A2', 'B1', 'B2', 'C1']
        level_masteries = []
        
        for level in levels:
            stats = self._calculate_mastery_for_level(level, cursor)
            level_masteries.append(stats['mastery_value'])
        
        global_mastery = sum(level_masteries) / len(level_masteries) if level_masteries else 0
        
        return global_coverage, global_mastery
