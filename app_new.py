...
class ImprovedAdaptiveLearningEngine:
    ...
    def ensure_database_schema(self):
        ...
        try:
            ...
            # UPDATED: Ensure answer_history has attempt type tracking
    ...
    def _calculate_mastery_for_level(self, level, cursor):
        """Calculate coverage and mastery for a specific level using rolling window."""
        cursor.execute("SELECT COUNT(*) AS total FROM questions WHERE cefr_level = ?", (level,))
        total_q = cursor.fetchone()['total']
        ...
        for qid in all_question_ids:
            mastery_score = self.calculate_single_question_mastery(qid, cursor)
            if mastery_score is not None:
                question_scores.append(mastery_score)
                attempted_count += 1
        ...
        final_mastery = mastery_with_coverage * 1.25
    ...
    def update_performance(self, cursor, question_id, cefr_level, topic, is_correct, is_freeform=False, is_partial=False, is_unanswered=False):
        ...
        # UPDATED: Log detailed attempt information to answer_history
        cursor.execute("""
...
- Free-form correct: +1.2 points
- Multiple choice correct: +0.6 points
- Free-form (partially correct): +0.8 points
...
Your level mastery is the average of all attempted questions, multiplied by coverage, with a 1.25× bonus to reward thoroughness without requiring perfection.
...
class StatsScreen(ctk.CTkFrame):
    ...
    def update_explanations(self):
        ...
        mastery_text = ctk.CTkLabel(mastery_frame, 
                                    text="Uses ONLY your last 5 attempts per question! Early mistakes fade away. Each attempt gets scored: FF correct +1.2, MC correct +0.6, Partial +0.8, FF wrong -1.4, MC wrong -1.8, Unanswered -0.2. A confidence multiplier (60%-100%) is applied based on attempt count. Final score per question is capped at +1.2 to -1.5.",
                                    font=ctk.CTkFont(size=12), wraplength=400, justify="left")
