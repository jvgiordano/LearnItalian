...
def enable_trackpad_scroll(scrollable_frame):
    """
    Enable trackpad/mousewheel scrolling for CTkScrollableFrame on Linux and Windows.
    Call this after creating each CTkScrollableFrame.
    """
    def on_mousewheel(event):
        # Linux uses Button-4 (scroll up) and Button-5 (scroll down)
        # Also handle standard MouseWheel event for compatibility
        if event.num == 4:
            scrollable_frame._parent_canvas.yview_scroll(-1, "units")
        elif event.num == 5:
            scrollable_frame._parent_canvas.yview_scroll(1, "units")
        elif hasattr(event, 'delta'):
            # Windows/Mac style delta
            scrollable_frame._parent_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")
    
    # Bind to the canvas and all child widgets
    canvas = scrollable_frame._parent_canvas
    
    # Linux-specific bindings (Button-4 and Button-5)
    canvas.bind("<Button-4>", on_mousewheel)
    canvas.bind("<Button-5>", on_mousewheel)
    
    # Also bind MouseWheel for cross-platform support (Windows/Mac)
    ...
    # Store reference to rebind when children are added
...
class HomeScreen(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        attribution_label = ctk.CTkLabel(self, text="(V1.1.0 - Made by jvgiordano using Claude 4.0 Sonnet, Claude 4.5 Opus, Gemini 2.5 Pro, Gemini 3.0 Pro, and Grok 4)", 
...
"""
        ...
        # Bind scroll to children after adding content
...
class TopicSelectionScreen(ctk.CTkFrame):
    ...
    def refresh_data(self):
        ...
        # Create UI elements for each level and its topics
...
class StatsScreen(ctk.CTkFrame):
    ...
    def update_progress_timeline(self):
        ...
        # Create the plot with standard DPI and Size
        ...
        # Determine tick positions based on number of data points
        num_dates = len(dates)
        ...
        # Use FixedLocator to prevent matplotlib from adding extra ticks
        ax.xaxis.set_major_locator(plt.FixedLocator(tick_positions))
        
        # Set x-axis limits to prevent extra space
        ...
        # Apply tight layout to ensure proper sizing and no cutoff
        fig.tight_layout()
        
        # Pack with center alignment and minimal padding
        ...
        # Ensure it doesn't expand indefinitely but stays centered
    ...
    def update_graph(self):
        ...
        if not history:
            ...
            # Create figure with standard size and DPI
            ...
            # Use FixedLocator to show only integer quiz numbers
            ...
            # Apply tight layout to ensure proper sizing
            fig1.tight_layout()
    ...
    def create_cefr_completion_chart(self, parent_frame):
        ...
        # Standard size and DPI for consistent scaling
